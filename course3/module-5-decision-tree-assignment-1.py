...
predictions == sample_validation_data['safe_loans']
...
small_prob_predictions = small_model.predict(sample_validation_data, output_type='probability')
...
accuracy = sum(predictions == validation_data[target])/float(len(validation_data))

positive_preds = (predictions == 1)
pairs = zip(positive_preds, validation_data[target])
fp_count = 0
for pair in pairs:
...
negative_preds = (predictions == -1)
pairs = zip(negative_preds, validation_data[target])
fn_count = 0
for pair in pairs:
    if pair[0] == 1 and pair[1] == 1:
        fn_count += 1
